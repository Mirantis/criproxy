...
var _ runtimeapi.RuntimeServiceServer = &FakeRuntimeServer19{}

func (r *FakeRuntimeServer19) SetFakeSandboxes(sandboxes []*FakePodSandbox19) {
	r.Lock()
	defer r.Unlock()

	r.Sandboxes = make(map[string]*FakePodSandbox19)
...
func (r *FakeRuntimeServer19) SetFakeContainers(containers []*FakeContainer19) {
	...
	r.Containers = make(map[string]*FakeContainer19)
...
func NewFakeRuntimeServer19(journal Journal, streamUrl string) *FakeRuntimeServer19 {
	...
	return &FakeRuntimeServer19{
		...
		Containers: make(map[string]*FakeContainer19),
		Sandboxes:  make(map[string]*FakePodSandbox19),
	}
}
...
func (r *FakeRuntimeServer19) RunPodSandbox(ctx context.Context, in *runtimeapi.RunPodSandboxRequest) (*runtimeapi.RunPodSandboxResponse, error) {
	...
	config := in.GetConfig()
	podSandboxID := BuildSandboxName19(config.Metadata)
	r.Sandboxes[podSandboxID] = &FakePodSandbox19{
...
func (r *FakeRuntimeServer19) CreateContainer(ctx context.Context, in *runtimeapi.CreateContainerRequest) (*runtimeapi.CreateContainerResponse, error) {
	...
	config := in.GetConfig()
	containerID := BuildContainerName19(config.Metadata, in.PodSandboxId)
	r.Containers[containerID] = &FakeContainer19{
